    ...
    const output: TransportOutput = adapter.encodeEffect(
      encodeInput({
        ...
        payload: {
          fields: {
            cwd: "/work",
            remoteConversationId: "/tmp/pi-session.jsonl",
          },
        },
        requestId: "request-1",
      }),
    ...
    expect(output.events).toEqual([
      {
        ConversationReady: {
          ...
          id: "conversation-1",
          remote: { Known: "/tmp/pi-session.jsonl" },
        },
      },
    ]);
  });

  it("passes through pi/event messages only", () => {
    const adapter = new PiEngineAdapter();
