...
export function useChatAttention(chatId: string): ChatAttention | null {
  const query = useChatAttentionList();
  return query.data?.find((attention) => attention.chatId === chatId) ?? null;
}
