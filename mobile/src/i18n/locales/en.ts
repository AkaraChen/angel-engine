import type { LocaleResource } from "./schema";

export const en = {
  translation: {
    app: {
      name: "Angel Engine",
    },
    common: {
      cancel: "Cancel",
      delete: "Delete",
      edit: "Edit",
      save: "Save",
      tryAgain: "Try again",
      newChat: "New chat",
      settings: "Settings",
      daemonOfflineHint: "The daemon may be offline or unreachable.",
      showLess: "Show less",
      showMore: "Show more",
    },
    login: {
      title: "Unlock Angel Engine",
      description:
        "Enter the pairing password set on your desktop app to connect this device.",
      passwordLabel: "Password",
      passwordPlaceholder: "Pairing password",
      passwordHelp: "Passwords are case-sensitive.",
      showPassword: "Show password",
      hidePassword: "Hide password",
      incorrectPassword: "Incorrect password. Try again.",
      connectionError:
        "Couldn't reach the desktop app. Check your connection and try again.",
      recoveryHint:
        "If you need a new pairing password, reset it in Desktop Settings → Mobile view, then return here.",
      connecting: "Connecting…",
      connect: "Connect",
    },
    shell: {
      backToChats: "Back to chats",
      titleChats: "Chats",
      titleChatFallback: "Chat",
    },
    sidebar: {
      home: "Home",
      navigationTitle: "Navigation",
      navigationDescription: "Primary destinations in Angel Engine.",
      close: "Close navigation",
    },
    daemonStatus: {
      unreachable: "Daemon unreachable",
      connecting: "Connecting to daemon…",
      online: "Daemon online · v{{version}}",
    },
    home: {
      emptyTitle: "No chats yet",
      emptyDescription: "Start a new agent session to see it here.",
      activityErrorTitle: "Couldn't load activity",
      errorTitle: "Couldn't load chats",
      filterSegments: "Filter chats",
      segmentEmpty: "Nothing here right now.",
      segments: {
        all: "All",
        attention: "Needs you",
        running: "Running",
        done: "Done",
      },
    },
    activity: {
      status: {
        waitingForYou: "Waiting for you",
        failed: "Failed",
        stuck: "Stuck",
        running: "Running",
        done: "Done",
      },
    },
    chat: {
      thinking: "Thinking…",
      turnFailed: "The assistant turn failed.",
      emptyTitle: "No messages yet",
      emptyDescription: "Send a message to start the conversation.",
      errorTitle: "Couldn't load this chat",
      runFailedTitle: "The last run failed",
      messagePlaceholder: "Message",
      sendAria: "Send",
      stopAria: "Stop",
      attachAria: "Attach files",
      attachments: "Attachments",
      removeAttachment: "Remove {{name}}",
      retryAttachment: "Retry {{name}}",
      roleUser: "You",
      roleAssistant: "Assistant",
      sendFailed:
        "Couldn't send that message. Your draft is still here — try again.",
      attachmentErrors: {
        accept: "That file type is not supported.",
        maxFileSize: "Each file must be 10 MB or smaller.",
        maxFiles: "You can attach up to 5 files.",
        fileRead: "Could not read that file. Try again or remove it.",
      },
      plan: "Plan",
      todo: "Todo",
      build: "Build",
      switchToPlan: "Switch to plan mode",
      switchToBuild: "Switch to build mode",
      planCreated: "{{title}} created",
      planUpdated: "{{title}} updated",
      planProgress: "{{completed}}/{{total}}",
      couldNotChangeMode: "Couldn't change mode",
      attentionNeedsInput: "Needs input",
      attentionNeedsInputDescription: "The agent is waiting for your response.",
      attentionReview: "Review",
    },
    elicitation: {
      defaultTitle: "The agent needs your input",
      allow: "Allow",
      allowForSession: "Allow for session",
      deny: "Deny",
      dismiss: "Dismiss",
      submit: "Submit",
      other: "Other",
      question: "Question",
      userInput: "User input",
      dynamicTool: "Dynamic tool",
      permissionProfile: "Permission profile",
      externalFlow: "External flow",
    },
    createChat: {
      description: "Start an agent session in a project or worktree.",
      promptLabel: "Initial prompt",
      promptPlaceholder: "What should the agent work on?",
      promptRequired: "Write a prompt first.",
      projectLabel: "Project",
      noProject: "No project (ad hoc)",
      agentLabel: "Agent",
      modelLabel: "Model",
      modelPlaceholder: "Default",
      reasoningLabel: "Reasoning",
      reasoningOptions: {
        default: "Default",
        minimal: "Minimal",
        low: "Low",
        medium: "Medium",
        high: "High",
      },
      worktreeTitle: "Run in a new worktree",
      worktreeDescription: "Isolate this chat in its own git worktree",
      worktreeHint: "Select a project to run in a worktree.",
      error:
        "Couldn't create the chat. Check the daemon connection and try again.",
      create: "Create chat",
    },
    settings: {
      appearance: {
        title: "Appearance",
        theme: "Theme",
        themeDescription: "Choose how the app looks on this device.",
        themeOptions: {
          system: "System",
          light: "Light",
          dark: "Dark",
        },
        language: "Language",
        languageDescription: "Choose the language for this device.",
        languageOptions: {
          en: "English",
          "zh-CN": "简体中文",
          "zh-TW": "繁體中文",
          fr: "Français",
          de: "Deutsch",
          ko: "한국어",
          ja: "日本語",
          es: "Español",
        },
      },
      connection: {
        title: "Connection",
        description:
          "This browser is paired with a local Angel Engine daemon. Disconnect only clears the token on this device.",
        server: "Paired server",
        sameOrigin: "This page's origin",
        status: "Status",
        statusOnline: "Connected",
        statusConnecting: "Connecting…",
        statusUnreachable: "Unreachable",
        daemonVersion: "Daemon version",
        versionUnknown: "Unknown",
        disconnectSectionTitle: "This device",
        disconnectDescription:
          "Disconnect removes only this browser's pairing token. It does not reset the desktop password or delete chats.",
        disconnect: "Disconnect this device",
        disconnectConfirmTitle: "Disconnect this device?",
        disconnectConfirmDescription:
          "You'll need the pairing password to connect again. Chats and the desktop password are unchanged.",
        disconnectConfirm: "Disconnect",
      },
      projects: {
        actionError: "Couldn't complete that project action. Try again.",
        add: "Add project",
        createTitle: "Add project",
        deleteAria: "Delete {{name}}",
        deleteChecking: "Checking linked chats…",
        deleteConflict:
          "The project changed since this dialog opened. Review the updated impact and try again.",
        deleteImpact:
          "Deleting this project will permanently delete {{count}} linked chats.",
        deleteImpactOne:
          "Deleting this project will permanently delete 1 linked chat.",
        deleteImpactUnknown:
          "Deleting this project will also permanently delete its linked chats.",
        deleteNoChats: "This project has no linked chats.",
        deleteTitle: "Delete project?",
        description:
          "Projects point to folders on the computer running Angel Engine.",
        editAria: "Edit {{name}}",
        editTitle: "Edit project",
        empty: "No projects yet.",
        filesKept: "Files in the project folder will be kept.",
        formDescription:
          "Enter an absolute folder path on the computer running Angel Engine.",
        loadError: "Couldn't load projects.",
        pathInvalid: "Enter a path to an existing folder.",
        pathLabel: "Folder path",
        pathPlaceholder: "/path/to/project",
        pathRequired: "Enter a folder path.",
        title: "Projects",
      },
      customAgents: {
        actionError: "Couldn't complete that custom-agent action. Try again.",
        add: "Add custom agent",
        argsLabel: "Arguments",
        argsPlaceholder: "One argument per line",
        autoAuthenticateLabel: "Authenticate automatically",
        commandLabel: "Command",
        commandRequired: "Enter a command.",
        createTitle: "Add custom agent",
        deleteAria: "Delete {{name}}",
        deleteChecking: "Checking linked chats…",
        deleteImpact:
          "Deleting this agent will permanently delete {{count}} linked chats.",
        deleteImpactOne:
          "Deleting this agent will permanently delete 1 linked chat.",
        deleteImpactUnknown:
          "Deleting this agent will also permanently delete its linked chats.",
        deleteNoChats: "This agent has no linked chats.",
        deleteTitle: "Delete custom agent?",
        description: "Manage lightweight ACP-compatible agent commands.",
        editAria: "Edit {{name}}",
        editTitle: "Edit custom agent",
        empty: "No custom agents yet.",
        environmentHint: "Environment values are stored locally in plain text.",
        environmentLabel: "Environment",
        environmentPlaceholder: "NAME=value, one per line",
        formDescription:
          "Configure the command and optional arguments used to start this agent.",
        loadError: "Couldn't load custom agents.",
        nameLabel: "Name",
        nameRequired: "Enter a name.",
        needAuthLabel: "Requires authentication",
        title: "Custom agents",
      },
      about: {
        appDescription: "Mobile companion for the Angel Engine desktop app.",
        appName: "Angel Engine Mobile",
        build: "Build",
        copied: "Copied",
        copyDiagnostics: "Copy diagnostics",
        copyFailed: "Couldn't copy. Copy the details by hand instead.",
        description:
          "These settings only affect this device and are kept separate from the desktop app's configuration.",
        diagnostics: "Diagnostics",
        title: "About",
      },
    },
  },
} satisfies LocaleResource;
