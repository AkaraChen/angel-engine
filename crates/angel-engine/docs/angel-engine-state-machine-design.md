# Angel Engine 状态机技术设计

本文设计一个 Rust 结构作为 ACP 和 Codex app-server 的 Angel Engine。它的目标不是把两个协议做成一组大而全的 enum mirror，而是把两边的请求、响应、通知、服务端回调翻译成同一个状态 reducer。
...
建议把 Angel Engine做成一个状态容器加一个纯 reducer。协议 transport、JSON-RPC id、反序列化类型都留在 adapter 层。
...
`ConversationId` 是 Angel Engine 自己的 id。`RemoteConversationId` 保留原协议 id：
...
Codex 的标准能力是 `steer = Supported`：`turn/steer` 向 active turn 增加输入，不创建新 turn。ACP 标准协议没有 steer 方法，因此 ACP 标准 adapter 应报告 `steer = Unsupported`；如果某个 ACP agent 或 proxy 通过扩展实现了 steer，就由该实现把能力表改成 `Extension(...)` 或 `Supported`。Angel Engine 只检查能力表，不按协议名硬编码 drop feature。

## 能力表

每个 Protocol adapter 必须在 initialize / session ready / thread ready 后给Angel Engine一份能力表。状态机只读能力表做 guard。
...
Codex 的 `ThreadStatus.activeFlags` 是 elicitation overlay 的摘要，不应该作为唯一事实来源。Angel Engine 应该以 ServerRequest 打开具体 elicitation，以 `serverRequest/resolved` 或响应写入完成关闭它。
...
Angel Engine reducer 只接受协议无关事件：
...
UI 不直接调用 ACP/Codex 方法，而是提交 `EngineCommand`：
...
| `EngineCommand` | ACP 接口 | 前置条件 | 产生的统一事件 |
| --- | --- | --- | --- |
...
| `EngineCommand` | Codex 接口 | 前置条件 | 产生的统一事件 |
| --- | --- | --- | --- |
...
2. `turn/steer` 必须带 `expectedTurnId`。Angel Engine应从 `active_turn` 读取；如果没有 active turn，直接拒绝。
3. `thread/unsubscribe` 只改 observer，不 close、不 cancel、不释放 thread 历史。
4. `thread/rollback` 只改 thread history，不 revert 文件系统。Angel Engine应在 `HistoryMutationResult` 中标记 `workspace_reverted: false`。
...
ACP `session/load` 有显式 replay/hydration 语义。Codex `thread/resume` 可能直接返回 turns，也可能配合 `thread/turns/list` 分页读取。Angel Engine统一映射到 `Hydrating`。
...
ACP 的 action 类型较通用，Codex 的 item 类型更细。Angel Engine要优先保留 `ActionKind`，同时把原始 protocol payload 放到 `raw` 或 `source` 字段，方便 UI 做协议特化展示。
...
Angel Engine 应该把“回复协议 request”与“状态已解除”拆开：
...
如果 `expectedTurnId` 不匹配，Codex 请求失败；Angel Engine应保持原 active turn，不创建新 turn。
...
UI 如果想提供“回滚代码”必须另做工作区 diff/revert；Angel Engine 不能把 `thread/rollback` 解释为文件已恢复。

## 错误和乱序处理

Angel Engine 需要显式处理这些情况：
...
1. `AngelEngine` 状态容器。
2. `EngineCommand` -> `ProtocolEffect`。
