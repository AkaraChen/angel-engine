...
use angel_engine::{
    AgentMode, AngelEngine, ApprovalPolicy, AvailableCommand, CommandPlan, ContentDelta,
    ContextPatch, ContextScope, ContextUpdate, ConversationCapabilities, ConversationId,
    ConversationLifecycle, ConversationState, ElicitationDecision, ElicitationKind,
    ElicitationPhase, ElicitationState, EngineCommand, EngineEvent, JsonRpcMessage,
    PermissionProfile, ProtocolFlavor, ProtocolTransport, ReasoningProfile, RuntimeState,
    SandboxProfile, SessionConfigOption, StartConversationParams, TransportClientInfo,
    TransportLog, TransportLogKind, TransportOptions, TurnId, UserAnswer, UserInput, UserQuestion,
    ...
    A: ProtocolTransport,
{
    pub fn start(
        ...
        Ok(Self {
            ...
            printed_plan_ready_hints: HashSet::new(),
        })
    }
    ...
    fn plan_ready_hints(&self, output: &angel_engine::TransportOutput) -> Vec<PlanReadyHint> {
        output
            .events
            .iter()
    ...
    fn record_plan_file_from_turn_output(
        &mut self,
        conversation_id: &ConversationId,
        turn_id: &TurnId,
    ) {
    ...
    fn record_plan_output_completion_hint(
        &mut self,
        conversation_id: &ConversationId,
        turn_id: &TurnId,
    ) {
    ...
    fn queue_plan_ready_hint(&mut self, hint: PlanReadyHint) {
        if !self
    ...
    fn print_plan_ready_hint_if_interactive(&mut self) -> io::Result<()> {
